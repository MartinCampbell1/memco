...
CREATE VIEW IF NOT EXISTS source_documents AS
SELECT
  id,
  workspace_id,
  source_path,
...
CREATE TABLE IF NOT EXISTS conversation_messages (
  ...
  speaker_person_id INTEGER REFERENCES persons(id) ON DELETE SET NULL,
...
CREATE VIRTUAL TABLE IF NOT EXISTS conversation_chunk_fts USING fts5(
  text,
  tokenize='porter unicode61'
);
...
CREATE TABLE IF NOT EXISTS memory_operations (
  ...
  target_fact_id INTEGER REFERENCES memory_facts(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS fact_candidates (
  ...
  publish_target_fact_id INTEGER REFERENCES memory_facts(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS review_queue (
  ...
  workspace_id INTEGER NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  person_id INTEGER REFERENCES persons(id) ON DELETE SET NULL,
  candidate_id INTEGER REFERENCES fact_candidates(id) ON DELETE SET NULL,
