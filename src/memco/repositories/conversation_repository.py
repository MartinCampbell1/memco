...
class ConversationRepository:
    ...
    def prepare_conversation_replace(self, conn, *, conversation_id: int) -> int:
        ...
        conn.execute("DELETE FROM conversation_speaker_map WHERE conversation_id = ?", (conversation_id,))
        conn.execute("DELETE FROM sessions WHERE conversation_id = ?", (conversation_id,))
        return source_id

    def replace_sessions(self, conn, *, conversation_id: int, source_id: int, sessions: list[dict]) -> dict[int, dict]:
        now = isoformat_z()
        ...
        for session in sessions:
            cursor = conn.execute(
                ...
                INSERT INTO sessions (
                    conversation_id, source_id, session_index, session_uid, started_at, ended_at,
                    detection_method, meta_json, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    conversation_id,
                    ...
                    session.get("detection_method", "single"),
                    json.dumps(session.get("meta", {}), ensure_ascii=False),
                    now,
                    now,
                ),
            )
            inserted[int(session["session_index"])] = {
                **session,
    ...
    def replace_messages(self, conn, *, conversation_id: int, messages: list[dict]) -> None:
        conversation_row = conn.execute("SELECT source_id FROM conversations WHERE id = ?", (conversation_id,)).fetchone()
        if conversation_row is None:
            raise ValueError("Unknown conversation")
        source_id = int(conversation_row["source_id"])
        now = isoformat_z()
        for index, message in enumerate(messages):
            cursor = conn.execute(
                ...
                INSERT INTO conversation_messages (
                    conversation_id, session_id, message_index, role, speaker_label, speaker_key,
            ...
            conn.execute(
                ...
                (
                    ...
                    json.dumps(
                        {
                            "message_index": index,
                            "speaker_key": message.get("speaker_key", ""),
                            "session_uid": message.get("session_uid", ""),
                        },
                        ensure_ascii=False,
                    ),
                    message.get("occurred_at", ""),
        ...
        for index, chunk in enumerate(chunks):
            cursor = conn.execute(
                ...
                INSERT INTO conversation_chunks (
                    conversation_id, source_id, session_id, chunk_index, start_message_index, end_message_index,
